...
import { Clock, Calendar, MapPin, ArrowLeft, CheckCircle2, AlertCircle, ShieldCheck, Phone, FileText } from 'lucide-react';
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-10 space-y-8">
      ...
      <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        {!booking.bookingStatus.startsWith('CANCELLED') && booking.bookingStatus !== 'COMPLETED' && (
            ...
            {steps.map((st, i) => (
              <div key={st.status} className="space-y-1">
                <div className={`w-8 h-8 rounded-full mx-auto flex items-center justify-center font-bold text-xs ${
                  booking.bookingStatus === st.status
                    ? 'bg-brand-500 text-white ring-4 ring-brand-100 dark:ring-brand-900'
                    : 'bg-slate-100 text-slate-400 dark:bg-slate-800'
                }`}>
                  {i + 1}
                </div>
                <span className="font-semibold text-slate-700 dark:text-slate-300 block">{st.label}</span>
              </div>
            ))}
          </div>
        </CardContent>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          <Card>
            ...
            <CardContent className="p-6 space-y-4 text-xs">
              ...
              <div className="flex justify-between border-b border-slate-100 dark:border-slate-800 pb-2">
                ...
                <span className="font-bold text-slate-900 dark:text-white">{booking.address?.fullAddress}, {booking.address?.city}</span>
              </div>
          ...
          {booking.attachments && booking.attachments.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle className="text-base font-bold">Foto Lampiran Kerusakan (Cloudflare R2)</CardTitle>
              </CardHeader>
              <CardContent className="p-6">
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base font-bold">Lini Masa Status (Audit Log)</CardTitle>
            </CardHeader>
        ...
        {/* Right Column: Pricing & Payment Status */}
