...
function ServiceCatalogContent() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 space-y-10">
      ...
      {/* WOW Hero Banner with Framer Motion */}
      <FadeIn direction="up">
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-slate-900 via-brand-950 to-brand-900 text-white p-8 sm:p-14 shadow-2xl border border-slate-800">
          ...
          <div className="relative z-10 max-w-3xl mx-auto text-center space-y-5">
            ...
            {/* Glassmorphism Floating Search Input */}
          ...
          <button
            ...
            className={`px-4 py-2.5 rounded-xl text-xs font-bold whitespace-nowrap transition-all cursor-pointer flex items-center gap-2 ${
              selectedCategory === ''
                ...
                : 'glass-pill text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800'
            }`}
          >
      ...
      {/* Services Grid with Loading Spinner & Framer Motion Stagger */}
      {loading ? (
        <div className="text-center py-20 glass-card rounded-3xl">
          <LoadingSpinner size="lg" text="Memuat katalog layanan profesional..." />
        </div>
      ) : services.length === 0 ? (
        ...
        <StaggerContainer className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {services.map((srv) => (
            <StaggerItem key={srv.id}>
              <Card
                className="flex flex-col justify-between group hover:-translate-y-1.5 hover:shadow-xl hover:border-brand-400/50 transition-all duration-200 glass-card rounded-2xl overflow-hidden h-full"
              >
                <CardContent className="p-6 space-y-4 flex-1 flex flex-col justify-between">
                  ...
                  <div className="space-y-4 pt-4 border-t border-slate-100 dark:border-slate-800 mt-4">
                    ...
                    <Link href={`/book/create?serviceId=${srv.id}`} className="block w-full">
  ...
  return (
    <Suspense fallback={
      <div className="text-center py-20">
        <LoadingSpinner size="lg" text="Memuat katalog..." />
      </div>
    }>
